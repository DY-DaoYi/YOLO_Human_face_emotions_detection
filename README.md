# 😊 基于 YOLO 的人脸情绪检测系统 (Human Face Emotions Detection System)
...
本项目是一个功能完整的人脸情绪智能检测系统。基于最新的 Ultralytics YOLO 系列模型构建，能够识别 **8种人脸情绪**（愤怒、满足、厌恶、恐惧、开心、中性、悲伤、惊讶）。本项目提供了美观、易用的 Streamlit Web 交互界面，非常适合作为**本科毕业设计**的参考与展示。
...
> 3.  **（赠品）数据集**：本项目使用 [Roboflow Human Face Emotions Dataset](https://universe.roboflow.com/emotions-dectection/human-face-emotions/dataset/30) 的开源数据集。
...
*   **🇨🇳 全中文支持**: 从界面菜单到检测框标签（如"愤怒"、"开心"、"悲伤"等情绪），全部采用中文显示，符合国内用户习惯。
...
为了让系统能够识别人脸情绪，你需要加载训练好的 YOLO 模型文件（`.pt`）。
...
2.  **放置模型**：将你训练得到的 `best.pt` 文件复制到 `models/` 文件夹中（建议重命名为清晰的名称，如 `yolo11m_face_emotions.pt`）。
...
本项目使用 [Roboflow Human Face Emotions Dataset](https://universe.roboflow.com/emotions-dectection/human-face-emotions/dataset/30) 开源数据集，主要用于检测**人脸情绪**。
...
2.  **训练规模**:
    ...
    *   **训练轮数**: 100 Epochs / 模型
3.  **实际耗时与成本**:
    *   *总耗时*: 29 小时 37 分 (约 29.63 小时)
    *   *云服务器费率*: ¥9.35 / 小时
    *   **总算力成本**: **29.63 小时 x ¥9.35 ≈ ¥277.04**

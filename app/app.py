            ...
            if st.button("开始检测", type="primary"):
                with st.spinner("检测中..."):
                    ...
                    if model:
                        ...
                        if len(boxes) > 0:
                            st.success(f"检测到 {len(boxes)} 个情绪")
                            # 创建简单的结果 DataFrame
                            data = []
                            for box in boxes:
                                cls = int(box.cls[0])
                                ...
                                data.append({"情绪类型": cn_name, "置信度": f"{conf:.2f}"})
...
with tab2:
    ...
    if 'results_data' in st.session_state and st.session_state['results_data']:
        ...
        if total_class_counts:
            st.subheader("😊 各类别情绪统计")
            class_stats_df = pd.DataFrame([
                {"情绪类型": cls, "检测数量": cnt} for cls, cnt in sorted(total_class_counts.items(), key=lambda x: x[1], reverse=True)
            ])
...
with tab4:
    ...
    本项目是基于 **YOLO (You Only Look Once)** 系列模型的人脸情绪检测系统，专为识别8种人脸情绪（愤怒、满足、厌恶、恐惧、开心、中性、悲伤、惊讶）而设计。

    ### ✨ 主要功能
    - **多模型兼容**: 无缝支持 YOLOv8, YOLO11, 以及最新的 YOLO26 模型。
    - **高精度实时检测**: 针对8种人脸情绪进行了深度优化和专门训练，支持图片和视频的快速检测。
